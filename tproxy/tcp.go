...
func ListenTCP(network string, laddr *net.TCPAddr) (net.Listener, error) {
	...
	/* we don't need this cause we pass tcp4 or tcp6 to distinguish between 4 and 6
	if ip := laddr.IP.To4(); nil == ip { // it's a ipv6 socket...


		if err = syscall.SetsockoptInt(int(fileDescriptorSource.Fd()), syscall.IPPROTO_IPV6, syscall.IPV6_V6ONLY, 1); err != nil {
			return nil, &net.OpError{Op: "listen", Net: network, Source: nil, Addr: laddr, Err: fmt.Errorf("set socket option: IPV6_V6ONLY: %s", err)}
		}
	*/
